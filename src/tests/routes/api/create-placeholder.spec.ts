...
describe("Create a placeholder image", () => {
    ...
    it("should responde with the created image (with width, height, text, color, background)", async (): Promise<void> => {
    ...
    it("should not responde with the created image (invalid any of: width, height, text, color, background)", async (): Promise<void> => {
        const response: supertest.Response = await request.get(
            "/create?width=abc&height=xyz&text=Testinggggggggggggggggggggggggggggggggggg&color=1234&background=reg(100,100,100)"
        );
